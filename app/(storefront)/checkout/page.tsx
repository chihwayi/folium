...
export default async function CheckoutPage({
  ...
  const { cancelled, payment_processing: paymentProcessing } =
    await searchParams;
  ...
  return (
    <main className="mx-auto w-full max-w-6xl flex-1 px-6 py-12 lg:py-16">
      ...
      {cancelled === "1" && (
        <p className="mt-5 rounded-lg border border-accent bg-accent/10 px-4 py-3 text-sm">
          Checkout was cancelled. Your cart is still here when you are ready.
        </p>
      )}
      {paymentProcessing === "1" && (
        <p className="mt-5 rounded-lg border border-accent bg-accent/10 px-4 py-3 text-sm">
          Your payment is still being confirmed. Please wait a moment before
          trying again.
        </p>
      )}
      <div className="mt-9 grid gap-10 lg:grid-cols-[1fr_420px]">
        <section className="rounded-xl border bg-card p-6 sm:p-8">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h2 className="font-serif text-2xl">Contact and delivery</h2>
              <p className="mt-1 text-sm leading-6 text-muted-foreground">
                Enter your email here. Stripe’s secure checkout will collect and
                validate your shipping address next.
              </p>
            </div>
          </div>
          <CheckoutForm />
        ...
        <aside className="rounded-xl border bg-card p-6">
          ...
          <ul className="mt-5 space-y-5">
            {items.map((item) => (
              <li
                key={item.bookId}
                className="grid grid-cols-[52px_1fr_auto] gap-3"
              >
                <BookCover
