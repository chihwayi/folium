...
export async function saveBook(formData: FormData) {
  ...
  await db.transaction(async (tx) => {
    ...
    if (input.id) {
      ...
      await tx
        .update(inventory)
        .set({ stockQuantity: input.stockQuantity, updatedAt: new Date() })
        .where(eq(inventory.bookId, input.id));
    } else {
      const [book] = await tx
        .insert(books)
        .values(values)
      ...
      await tx
        .insert(inventory)
        .values({ bookId: book.id, stockQuantity: input.stockQuantity });
    }
  });
...
function r2Client() {
  ...
  return new S3Client({
    ...
    credentials: {
      accessKeyId: z.string().min(1).parse(process.env.R2_ACCESS_KEY_ID),
      secretAccessKey: z
        .string()
        .min(1)
        .parse(process.env.R2_SECRET_ACCESS_KEY),
    },
  });
}
...
export async function uploadCover(formData: FormData) {
  ...
  if (
    ...
    throw new Error(
      "Cover must be JPEG, PNG, WebP, or AVIF and no larger than 5 MB",
    );
  const bucket = z.string().min(1).parse(process.env.R2_BUCKET_NAME);
  const publicUrl = z.url().parse(process.env.R2_PUBLIC_URL);
...
export async function deleteCover(key: string) {
  ...
  const safeKey = z
    .string()
    .regex(/^covers\/[a-f0-9-]+\.(?:jpe?g|png|webp|avif)$/)
    .parse(key);
  await r2Client().send(
    new DeleteObjectCommand({
      Bucket: z.string().min(1).parse(process.env.R2_BUCKET_NAME),
      Key: safeKey,
    }),
  );
}
...
export async function updateStock(formData: FormData) {
  await requireAdmin();
  const input = stockSchema.parse(Object.fromEntries(formData));
  await db
    .update(inventory)
    .set({
...
export async function updateOrderStatus(formData: FormData) {
  ...
  if (
    !order ||
    !(transitions[order.status] as readonly string[]).includes(input.status)
  )
...
export async function updateUserRole(formData: FormData) {
  ...
  if (input.id === actor.id && input.role !== "owner")
    throw new Error("Owners cannot remove their own access");
  await db
    .update(users)
...
export async function saveCollection(formData: FormData) {
  await requireAdmin();
  const input = collectionSchema.parse(Object.fromEntries(formData));
  const values = {
    title: input.title,
    slug: input.slug,
    ...
    isPublished: input.isPublished === "on",
    updatedAt: new Date(),
  };
  if (input.id)
    await db
...
const collectionBookSchema = z.object({
  collectionId: z.uuid(),
  bookId: z.uuid(),
  position: z.coerce.number().int().nonnegative(),
});
export async function addCollectionBook(formData: FormData) {
  await requireAdmin();
  const input = collectionBookSchema.parse(Object.fromEntries(formData));
  await db
    .insert(collectionBooks)
    .values(input)
    .onConflictDoUpdate({
      target: [collectionBooks.collectionId, collectionBooks.bookId],
...
export async function removeCollectionBook(formData: FormData) {
  await requireAdmin();
  const id = uuid.parse(formData.get("id"));
  await db.delete(collectionBooks).where(eq(collectionBooks.id, id));
  revalidatePath("/admin/collections");
  revalidatePath("/");
}
...
export async function moderateReview(formData: FormData) {
  await requireAdmin();
  const input = z
    .object({ id: z.uuid(), approved: z.enum(["true", "false"]) })
    .parse(Object.fromEntries(formData));
  await db
    .update(reviews)
    .set({ isApproved: input.approved === "true", updatedAt: new Date() })
...
export async function savePromoCode(formData: FormData) {
  ...
  if (input.type === "percentage" && input.value > 99)
    throw new Error("Percentage discounts must be between 1 and 99");
  const expiresAt = input.expiresAt ? new Date(input.expiresAt) : null;
  if (expiresAt && Number.isNaN(expiresAt.getTime()))
    throw new Error("Enter a valid expiry date");
  const usageLimit = input.usageLimit
    ? z.coerce.number().int().positive().parse(input.usageLimit)
    : null;
  const values = {
    code: input.code,
    description: input.description || null,
  ...
  else await db.insert(promoCodes).values(values);
  revalidatePath("/admin/promotions");
}

export async function archivePromoCode(formData: FormData) {
  await requireAdmin();
  const id = uuid.parse(formData.get("id"));
  await db
    .update(promoCodes)
