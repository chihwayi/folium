    ...
    <>
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-muted-foreground">Create, edit, and import books.</p>
        </div>
        <Button asChild>
      ...
      <div className="mt-6 rounded-xl border bg-card p-4">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          The template includes an example row and a list of your current category IDs — fill it in and re-upload
          below. <code>coverImageUrl</code> must already point to a hosted image; the template explains why.
        </p>
        <form action={importBooks} className="mt-4 flex flex-wrap items-center gap-3">
      ...
      <div className="mt-8 divide-y rounded-xl border bg-card">
        {rows.map((book) => (
          <div key={book.id} className="flex items-center justify-between gap-4 p-5">
            <div>
              <p className="font-medium">{book.title}</p>
              <p className="text-sm text-muted-foreground">
                {book.author} · {formatPrice(book.price)} · {book.stock} in stock
              </p>
            </div>
