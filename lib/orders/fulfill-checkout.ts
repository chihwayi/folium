...
function parseLines(session: Stripe.Checkout.Session): PaidLine[] {
  ...
  return lines.map((line) => {
    ...
    const productObject = typeof product === "string" || !product || product.deleted ? null : product;
    ...
    const unitPriceCents = z.number().int().nonnegative().parse(line.price?.unit_amount);
...
export async function fulfillCheckout(sessionId: string) {
  ...
  if (session.mode !== "payment" || session.payment_status !== "paid") throw new Error("Checkout Session is not paid");

  const shipping = session.collected_information?.shipping_details;
  const email = session.customer_details?.email;
  const amountTotal = session.amount_total;
  if (!shipping || !email || amountTotal === null) throw new Error("Checkout Session is missing customer or shipping details");
  const address = shippingAddressSchema.parse(shipping.address);

  const lines = parseLines(session);
  const calculatedTotal = lines.reduce((sum, line) => sum + line.unitPriceCents * line.quantity, 0);
  if (calculatedTotal !== amountTotal) throw new Error("Checkout Session total does not match its line items");

  const userIdResult = session.metadata?.userId ? userIdSchema.safeParse(session.metadata.userId) : null;
  ...
  const orderId = await db.transaction(async (tx) => {
    ...
    const [order] = await tx.insert(orders).values(orderValues).returning({ id: orders.id });

    for (const line of lines) {
      const [updated] = await tx.update(inventory).set({
        stockQuantity: sql`${inventory.stockQuantity} - ${line.quantity}`,
        updatedAt: new Date(),
      }).where(and(eq(inventory.bookId, line.bookId), gte(inventory.stockQuantity, line.quantity))).returning({ id: inventory.id });
      if (!updated) throw new Error(`Insufficient inventory for book ${line.bookId}`);
    }
